(() => {
  ...
  const CLOSE_ANIMATION_NAME = "search-window-exit";
  ...
  const init = () => {
    ...
    const close = () => {
      ...
      // Capture an interrupted entrance so closing never snaps to full size.
      const currentStyle = getComputedStyle(windowNode);
      windowNode.style.setProperty('--search-close-opacity', currentStyle.opacity);
      windowNode.style.setProperty('--search-close-transform', currentStyle.transform);
      windowNode.classList.remove("is-opening");
      if (windowNode.contains(document.activeElement)) {
        state.returnFocus?.focus({ preventScroll: true });
      }
      windowNode.inert = true;
    ...
    const open = async (event) => {
      const wasHidden = windowNode.hidden;
      const wasClosing = state.isClosing;
      const currentStyle = wasClosing ? getComputedStyle(windowNode) : null;
      windowNode.style.setProperty('--search-open-opacity', currentStyle?.opacity || '0');
      windowNode.style.setProperty('--search-open-transform', currentStyle?.transform || 'translateY(-6px) scale(0.975)');
      clearCloseAnimation();
      state.returnFocus = event?.currentTarget || document.activeElement;
      windowNode.inert = false;
      ...
      if (!state.hasPosition) {
        // Position before painting or animating to avoid flashing at the left.
        const width = windowNode.offsetWidth;
        const height = windowNode.offsetHeight;
        const left = window.innerWidth - width - WINDOW_MARGIN;
        const top = Math.max(WINDOW_MARGIN, 72);
        setWindowPosition(windowNode, left, top, width, height);
      }

      if ((wasHidden || wasClosing) && !prefersReducedMotion()) windowNode.classList.add("is-opening");
    ...
    windowNode.addEventListener("animationend", (event) => {
      if (event.target === windowNode && event.animationName === "search-window-enter") {
        windowNode.classList.remove("is-opening");
      }
    });
